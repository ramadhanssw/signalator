...
class sttHistoryPage extends StatefulWidget {
  const sttHistoryPage({Key? key}) : super(key: key);

  @override
  _sttHistoryPageState createState() => _sttHistoryPageState();
}

class _sttHistoryPageState extends State<sttHistoryPage> {
  final FlutterTts flutterTts = FlutterTts();
  final currentUser = FirebaseAuth.instance.currentUser;
  CollectionReference sttHistory =
      FirebaseFirestore.instance.collection("sttHistory");

  Future<void> _deleteHistory(String historyId) async {
    await sttHistory.doc(historyId).delete();
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Speech To Text History'),
